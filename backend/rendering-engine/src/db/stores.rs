use super::common::*;
use crate::types::database::Interface;
use shared::{
    constants::{BIN_ID, ROOT_ID},
    types::{Asset, Directory, FileSystemEntry},
};
...
pub fn get(db: &DatabaseManager, store_id: u32) -> Result<Vec<FileSystemEntry>> {
    ...
    let mut result: Vec<FileSystemEntry> = directories
        ...
        .collect();
    result.extend(assets.into_iter().map(FileSystemEntry::Asset));
