...
pub fn process(image_path: &PathBuf, hdf5_path: &PathBuf) -> Result<()> {
    ...
    let num_tiles = (image_dimensions.w as usize / TILE_SIZE_USIZE)
        * (image_dimensions.h as usize / TILE_SIZE_USIZE);
...
pub fn process(image_path: &PathBuf, hdf5_path: &PathBuf) -> Result<()> {
    ...
    for handle in handles {
        ...
        let _ = row_jpeg_tiles
            .iter()
            .for_each(|jpeg_tile| jpeg_tiles.push([VarLenArray::from_slice(&jpeg_tile)]));
    }
